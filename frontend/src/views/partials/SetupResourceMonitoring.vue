...
<template>
  <teleport to="body">
    <ModalDialog :close-modal="closeModal" :is-open="isModalOpen" :key="serverId + '_setup_resource_monitoring_modal'"
      width="2xl">
      <template v-slot:header>Setup Resource Monitoring</template>
      <template v-slot:body>
        <div class="mt-6">
          <Step title="Install swiftwave-stats-ninja package"
            sub-title="This package is responsible for collecting server and running container metrics" prefix-text="1"
            type="primary" show-body>
            <div>
              <p class="font-medium">For Debian/Ubuntu users,</p>
              <Code :content="['sudo mkdir -p /etc/apt/keyrings', 'sudo mkdir -p /root/.gnupg',
                ...
                'echo deb [signed-by=/etc/apt/keyrings/swiftwave.gpg] http://deb.repo.swiftwave.org/swiftwave-stats-ninja stable',
                'sudo tee /etc/apt/sources.list.d/swiftwave-stats-ninja.list', 'sudo apt update'
                , 'sudo apt install swiftwave-stats-ninja=2.0.0']" />
              <div class="my-4"></div>
              <p class="font-medium">For Fedora/CentOS/Almalinux/RockyLinux users,</p>
              <Code
                :content="['sudo dnf config-manager --add-repo http://rpm.repo.swiftwave.org/swiftwave.repo', 'sudo dnf install -y swiftwave-stats-ninja-2.0.0-1']" />
            </div>
          </Step>
          <Step title="Enable swiftwave-stats-ninja"
            sub-title="This will start the swiftwave-stats-ninja service and enable it to start on boot" prefix-text="2"
            type="primary" show-body>
            <p v-if="isFetchingAnalyticsServiceToken" class="italic">Fetching stats-ninja authentication token ...</p>
            <Code v-else :content="[`sudo swiftwave-stats-ninja enable ${managementAddress} ${analyticsToken}`]" />
          </Step>
        </div>
      </template>
