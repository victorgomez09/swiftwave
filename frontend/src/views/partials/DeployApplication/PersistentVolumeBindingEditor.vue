...
<template v-slot:message>
      <TableMessage v-if="persistentVolumeBindingKeys.length === 0" class="flex flex-col items-center">
        ...
        <FilledButton class="mt-3 max-w-fit" @click="addPersistentVolumeBinding">Add Persistent Volume Binding
        </FilledButton>
      </TableMessage>
      <div v-else class="flex flex-col gap-2 px-6 py-2 text-sm text-gray-600">
        <p class="m-0 inline-flex items-center p-0">
          <FilledButton slim @click="addPersistentVolumeBinding" class="mr-2">Add Persistent Volume Binding
          </FilledButton>
      ...
      <tr v-for="key in persistentVolumeBindingKeys" :key="key">
        <TableRow>
          <select :key="`credential-${key}`"
            ...
            <option v-for="volume in persistentVolumes" :key="volume.id"
              :selected="volume.id === persistentVolumeBindingsMap[key]?.persistentVolumeID" :value="volume.id">
              {{ volume.name }}
            </option>
          </select>
        </TableRow>
        <TableRow>
          <input :key="`value-${key}`"
            class="focus:border-primary focus:ring-primary block w-full rounded-md border-gray-300 shadow-sm sm:text-sm"
            placeholder="Mounted Path" type="text" v-bind:value="persistentVolumeBindingsMap[key]?.mountingPath ?? ''"
            @input="(event) => onMountingPathChange(key, event.target.value)" />
        </TableRow>
        <TableRow align="right" class="flex">
          <FilledButton :key="`delete-${key}`" :click="() => deletePersistentVolumeBinding(key)" class="w-full"
            type="danger">
