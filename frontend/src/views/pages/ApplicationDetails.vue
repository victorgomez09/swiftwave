...
<template>
  ...
  <section v-else class="flex flex-col mx-auto w-full max-w-7xl h-full overflow-hidden">
    <div class="flex flex-row justify-between">
      ...
      <div>
        <div class="flex items-center">
          <div class="flex items-center gap-2 overflow-hidden">
            <div class="flex items-center justify-center gap-2">
              <h3 class="font-bold text-xl">{{ applicationDetails.name }}</h3>
              <div v-if="applicationDetails.latestDeployment.status === 'live'" class="badge badge-success">
                {{ applicationDetails.latestDeployment.status }}
              </div>
              <div v-else-if="applicationDetails.latestDeployment.status === 'pending'" class="badge badge-warning">
                {{ applicationDetails.latestDeployment.status }}
              </div>
              <div v-else-if="applicationDetails.latestDeployment.status === 'deployPending'"
                class="badge badge-warning">
                {{ applicationDetails.latestDeployment.status }}
              </div>
              <div v-else-if="applicationDetails.latestDeployment.status === 'deploying'" class="badge badge-warning">
                {{ applicationDetails.latestDeployment.status }}
              </div>
              <div v-else-if="applicationDetails.latestDeployment.status === 'failed'" class="badge badge-danger">
                {{ applicationDetails.latestDeployment.status }}
              </div>
              <div v-else-if="applicationDetails.latestDeployment.status === 'stopped'" class="badge badge-secondary">
                {{ applicationDetails.latestDeployment.status }}
              </div>
              <div v-else-if="applicationDetails.latestDeployment.status === 'stalled'" class="badge badge-secondary">
                {{ applicationDetails.latestDeployment.status }}
              </div>
              <div v-if="applicationDetails.isSleeping" class="badge badge-warning"> Sleeping</div>
            </div>
            <div @click="openApplicationGroupUpdateModal" class="badge cursor-pointer">
        ...
        <div class="mt-2 flex items-center gap-2 font-normal text-gray-800">
          ...
          <p v-else>
            <b class="text-warning-600">No Ingress Rules ! </b><i>(
              <RouterLink :to="{
                name: 'Application Details Ingress Rules',
                params: { id: $route.params.id }
              }" class="font-semibold text-info-600">Add ingress rules
              </RouterLink>
    ...
    <div class="flex flex-row flex-1 gap-5 w-full mt-8">
      ...
      <div class="w-full h-full">
        ...
        <div v-if="applicationUpdater.isConfigurationUpdated"
          ...
          <FilledButton :click="applicationUpdater.applyConfigurationChanges"
            :loading="applicationUpdater.isDeployRequestSubmitting" class="primary">
            Apply Changes
          </FilledButton>
          <FilledButton :click="applicationUpdater.cancelConfigurationChanges"
            :disabled="applicationUpdater.isDeployRequestSubmitting" class="secondary">
            Cancel
          </FilledButton>
        </div>
      </div>
