    ...
    <div class="h-full w-1/2">
      ...
      <div>
        <label class="form-control w-full max-w-xs">
          ...
          <input autocomplete="off" v-model="stateRef.stackName" class="input input-bordered w-full"
            placeholder="Enter stack name" type="text" />
        </label>
      </div>
      <!--   Stack Config (Yaml)   -->
      <div class="mt-3 h-[70%]">
        ...
        <div ref="editor" class="mt-1 h-full w-full overflow-hidden rounded-md border input-bordered" />
      </div>
    </div>
    <div class="w-1/2 pt-4">
      <p class="select-none text-base font-semibold">⚙️ Actions</p>
      <FilledButton class="mt-4 w-full" type="primary" :click="cleanupStackConfigHelper"
        :loading="isCleanupAndVerifyStackConfigLoading">Cleanup & Verify Stack Config
      </FilledButton>
      <!--   Verification Status   -->
      <div class="mt-2 p-3" v-if="stateRef.verificationStatus !== -1">
        <font-awesome-icon v-if="stateRef.verificationStatus === 1" icon="fa-solid fa-circle-check"
          class="mr-1 text-xl text-success" />
        <font-awesome-icon v-else-if="stateRef.verificationStatus === 0" icon="fa-solid fa-circle-xmark"
          class="mr-1 text-xl text-error" />
        {{ stateRef.verificationStatus === 1 ? 'Verified' : 'Verification Failed' }}
      </div>
      <!--    Verification Result  -->
      <div class="mt-2 whitespace-pre-line rounded-md border-2 border-error bg-error p-3" v-if="stateRef.error !== ''">
        {{ stateRef.error }}
      </div>
      <div class="mt-2 whitespace-pre-line rounded-md border-2 border-success bg-success p-3"
        v-if="stateRef.message !== ''">
        {{ stateRef.message }}
      </div>
      <!--   Deploy Button   -->
      <FilledButton v-if="stateRef.verificationStatus === 1" class="mt-6 w-full" type="primary"
        :loading="deployStackLoading" :click="deployStackHelper">
      ...
      <ModalDialog :is-open="isModalOpen" non-cancelable>
        ...
        <template v-slot:body>
          <div class="flex flex-col space-y-3 pt-3">
            <div class="flex items-center space-x-2" v-for="result in stateRef.deployedApplicationsResult"
              :key="result.application.id">
              <font-awesome-icon v-if="result.success" icon="fa-solid fa-circle-check"
                class="text-base text-success-500" />
              <font-awesome-icon v-else icon="fa-solid fa-circle-xmark" class="text-base text-error" />
              <p>
                {{ result.application?.name ?? 'N/A' }}
                <span v-if="result.message !== '' && !result.success"> - {{ result.message }}</span>
              </p>
              <FilledButton v-if="result.success" type="primary" slim :click="() =>
                openUrlInNewPage(
                  $router.resolve({
        ...
        <template v-slot:footer>
          <FilledButton type="warning" class="w-full" :click="() => $router.replace('/applications')">Go to Applications
            List
          </FilledButton>
        </template>
      </ModalDialog>
